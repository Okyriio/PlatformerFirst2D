...
void Game::Init()
{
    ...
    window_.create(sf::VideoMode(1400, 800),
        "SAE Platformer");
    window_.setVerticalSyncEnabled(true);
    //window_.setFramerateLimit(10);
    if (!wall_.loadFromFile("data/sprites/wall2.png"))
    {
        std::cerr << "Could not load texture!\n";
    }
    sprite_.setTexture(wall_);
    sprite_.setOrigin(wall_.getSize().x/2.0f, wall_.getSize().y/3.0f );
    player_.Init();
	sprite_.setScale(wallScale_,wallScale_);
    platform_.Init();
    platform_.Init2();
    platform_.Init3();
}
